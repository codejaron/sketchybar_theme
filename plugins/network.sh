...
DOWN_FORMAT=""
if [ "$DOWN" -gt "999" ]; then
	DOWN_FORMAT=$(echo "$DOWN" | awk '{ printf "%03.0f MB", $1 / 8000}')
else
	DOWN_FORMAT=$(echo "$DOWN" | awk '{ printf "%03.0f KB", $1/8}')
fi

UP_FORMAT=""
if [ "$UP" -gt "999" ]; then
	UP_FORMAT=$(echo "$UP" | awk '{ printf "%03.0f MB", $1 / 8000}')
else
	UP_FORMAT=$(echo "$UP" | awk '{ printf "%03.0f KB", $1/8}')
fi
